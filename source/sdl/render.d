...
mixin(joinFnBinds((){
	FnBind[] ret = [
		...
		{q{int}, q{SDL_CreateWindowAndRenderer}, q{int width, int height, uint windowFlags, SDL_Window** window, SDL_Renderer** renderer}},
		...
		{q{int}, q{SDL_RenderCopyEx}, q{SDL_Renderer* renderer, SDL_Texture* texture, const(SDL_Rect)* srcRect, const(SDL_Rect)* dstRect, const(double) angle, const(SDL_Point)* centre, const(SDL_RendererFlip) flip}},
	...
	if(sdlSupport >= SDLSupport.v2_0_10){
		FnBind[] add = [
			...
			{q{int}, q{SDL_RenderCopyExF}, q{SDL_Renderer* renderer, SDL_Texture* texture, const(SDL_Rect)* srcRect, const(SDL_FRect)* dstRect, const(double) angle, const(SDL_FPoint)* centre, const(SDL_RendererFlip) flip}},
	...
	if(sdlSupport >= SDLSupport.v2_0_18){
		{
			FnBind[] add = [
				...
				{q{int}, q{SDL_RenderGeometry}, q{SDL_Renderer* renderer, SDL_Texture* texture, const(SDL_Vertex)* vertices, int numVertices, const(int)* indices, int numIndices}},
				{q{int}, q{SDL_RenderSetVSync}, q{SDL_Renderer* renderer, int vsync}},
			];
			ret ~= add;
		}
		if(sdlSupport >= SDLSupport.v2_0_20){
			FnBind[] add = [
				{q{int}, q{SDL_RenderGeometryRaw}, q{SDL_Renderer* renderer, SDL_Texture* texture, const(float)* xy, int xyStride, const(SDL_Color)* colour, int colourStride, const(float)* uv, int uvStride, int numVertices, const(void)* indices, int numIndices, int sizeIndices}},
			];
			ret ~= add;
		}else{
			FnBind[] add = [
				{q{int}, q{SDL_RenderGeometryRaw}, q{SDL_Renderer* renderer, SDL_Texture* texture, const(float)* xy, int xyStride, const(int)* colour, int colourStride, const(float)* uv, int uvStride, int numVertices, const(void)* indices, int numIndices, int sizeIndices}},
			];
			ret ~= add;
		}
	}
