...
@safe @nogc nothrow pure {
    ...
    uint SDL_WINDOWPOS_ISUNDEFINED(uint x) { (x & 0xFFFF0000) == SDL_WINDOWPOS_UNDEFINED_MASK; }
    uint SDL_WINDOWPOS_CENTERED_DISPLAY(uint x) { SDL_WINDOWPOS_CENTERED_MASK | x; }
    uint SDL_WINDOWPOS_ISCENTERED(uint x) { (x & 0xFFFF0000) == SDL_WINDOWPOS_CENTERED_MASK; }
}
